...
const FIELD_DETECTION_AND_FILL_CODE = `
(function() {
...
test.describe('Generic Job Application Form', () => {
  test('should detect and fill all form fields from profile data', async ({ page }) => {
    const fixturePath = path.resolve(__dirname, 'fixtures/generic-form.html');
    await page.goto(`file://${fixturePath}`);

    await page.addScriptTag({ content: FIELD_DETECTION_AND_FILL_CODE });
    ...
    expect(fieldTypes).toContain('linkedinUrl');
    expect(fieldTypes).toContain('githubUrl');
    ...
    await expect(page.locator('#email')).toHaveValue('jane.smith@example.com');
    await expect(page.locator('#phone')).toHaveValue('+1 (555) 123-4567');
    await expect(page.locator('#linkedin_url')).toHaveValue('https://linkedin.com/in/janesmith');
    await expect(page.locator('#github_url')).toHaveValue('https://github.com/janesmith');
  });

  test('should handle partial profile data gracefully', async ({ page }) => {
    const fixturePath = path.resolve(__dirname, 'fixtures/generic-form.html');
    await page.goto(`file://${fixturePath}`);
  ...
  test('should correctly split name into first and last name', async ({ page }) => {
    const fixturePath = path.resolve(__dirname, 'fixtures/generic-form.html');
    await page.goto(`file://${fixturePath}`);
    ...
    await expect(page.locator('#first_name')).toHaveValue('Alexander');
    await expect(page.locator('#last_name')).toHaveValue('Christopher Johnson');
  });
});
