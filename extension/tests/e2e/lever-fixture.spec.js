...
const MOCK_PROFILE = {
  ...
  portfolio: 'https://davidmartinez.com',
  experiences: [],
};

test.describe('Lever Application Form', () => {
  test('should detect and fill Lever-style form fields', async ({ page }) => {
    const fixturePath = path.resolve(__dirname, 'fixtures/lever-form.html');
    ...
    expect(fieldTypes).toContain('fullName');
    ...
    await expect(page.locator('#name')).toHaveValue('David Martinez');
    await expect(page.locator('#email')).toHaveValue('david.martinez@example.com');
    await expect(page.locator('#phone')).toHaveValue('+1 (555) 456-7890');
  });

  test('should fill LinkedIn and GitHub profile URLs in Lever format', async ({ page }) => {
    const fixturePath = path.resolve(__dirname, 'fixtures/lever-form.html');
    await page.goto(`file://${fixturePath}`);
    ...
    await expect(page.locator('#linkedin')).toHaveValue('https://linkedin.com/in/davidmartinez');
    await expect(page.locator('#github')).toHaveValue('https://github.com/davidmartinez');
  });

  test('should fill portfolio URL in Lever form', async ({ page }) => {
    const fixturePath = path.resolve(__dirname, 'fixtures/lever-form.html');
    await page.goto(`file://${fixturePath}`);
    ...
    await expect(page.locator('#portfolio')).toHaveValue('https://emmadev.io');
  });

  test('should handle full application with all Lever fields', async ({ page }) => {
    const fixturePath = path.resolve(__dirname, 'fixtures/lever-form.html');
    await page.goto(`file://${fixturePath}`);
    ...
    await expect(page.locator('#name')).toHaveValue('Sarah Wilson');
    await expect(page.locator('#email')).toHaveValue('sarah.wilson@example.com');
    await expect(page.locator('#phone')).toHaveValue('+1 (555) 999-8888');
    await expect(page.locator('#linkedin')).toHaveValue('https://linkedin.com/in/sarahwilson');
    await expect(page.locator('#github')).toHaveValue('https://github.com/sarahwilson');
    await expect(page.locator('#portfolio')).toHaveValue('https://sarahwilson.design');
  });

  test('should detect fields using label text as fallback', async ({ page }) => {
    const fixturePath = path.resolve(__dirname, 'fixtures/lever-form.html');
    await page.goto(`file://${fixturePath}`);
    ...
    const emailDetected = detectionResults.some(f => f.fieldType === 'email');
    const phoneDetected = detectionResults.some(f => f.fieldType === 'phone');
    const nameDetected = detectionResults.some(f => f.fieldType === 'fullName');

    expect(emailDetected).toBeTruthy();
    expect(phoneDetected).toBeTruthy();
    expect(nameDetected).toBeTruthy();
  });
});
