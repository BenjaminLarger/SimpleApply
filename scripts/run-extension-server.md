...
cleanup() {
    ...
    kill -- -$WXT_PID 2>/dev/null || kill $WXT_PID 2>/dev/null || true
...
# Wait for WXT to report success in its log (up to 30s)
for i in $(seq 1 30); do
    if grep -q "Built extension" /tmp/wxt-dev.log 2>/dev/null; then
        break
    fi
    if ! kill -0 $WXT_PID 2>/dev/null; then
        echo "❌ WXT Dev Server process exited"
        echo "   Check: cat /tmp/wxt-dev.log"
        exit 1
    fi
    sleep 1
done
...
if ! grep -q "Built extension" /tmp/wxt-dev.log 2>/dev/null; then
    echo "❌ WXT Dev Server failed to start (timeout)"
    echo "   Check: cat /tmp/wxt-dev.log"
    exit 1
fi
